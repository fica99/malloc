...
*********** Heap size **********************
*/
static const size_t FT_MAL_TINY_HEAP_DATA_SIZE = FT_MAL_TINY_CHUNK_SIZE * FT_MAL_NB_OF_TINY_CHUNKS;
static const size_t FT_MAL_SMALL_HEAP_DATA_SIZE = FT_MAL_SMALL_CHUNK_SIZE * FT_MAL_NB_OF_SMALL_CHUNKS;
static const size_t FT_MAL_TINY_HEAP_SIZE = FT_MAL_TINY_HEAP_DATA_SIZE + FT_MAL_HEAP_INFO_SIZE;
static const size_t FT_MAL_SMALL_HEAP_SIZE = FT_MAL_SMALL_HEAP_DATA_SIZE + FT_MAL_HEAP_INFO_SIZE;

// determine allocation size for heap
							...
							t_e_ft_mal_heap_type heap_type, size_t large_chunk_size)
{
	...
	if (heap_type == FT_MAL_TINY_HEAP_TYPE)
		total_size = FT_MAL_TINY_HEAP_SIZE;
	else if (heap_type == FT_MAL_SMALL_HEAP_TYPE)
		total_size = FT_MAL_SMALL_HEAP_SIZE;
	else if (heap_type == FT_MAL_LARGE_HEAP_TYPE)
		total_size = large_chunk_size + FT_MAL_CHUNK_SIZE + FT_MAL_HEAP_INFO_SIZE;
	else
		total_size = 0;

	// add arena metadata
	if (!arena)
		total_size += FT_MAL_STATE_SIZE;
...
static void				ft_mal_add_new_empty_chunks(t_s_ft_mal_state *arena, t_s_ft_mal_heap_info *heap_info)
{
	...
	i = FT_MAL_HEAP_INFO_SIZE;
	if ((void*)heap_info + i == arena)
		i += FT_MAL_STATE_SIZE;
	
	// initialize header
	ft_bzero((void*)heap_info + i, FT_MAL_CHUNK_SIZE);
	...
	if (heap_info->heap_type == FT_MAL_TINY_HEAP_TYPE)
	{
		...
		{
			// initialize header
			if (i + FT_MAL_CHUNK_SIZE < heap_info->total_size)
				ft_bzero((void*)heap_info + i, FT_MAL_CHUNK_SIZE);
		
			if (i + FT_MAL_TINY_CHUNK_SIZE < heap_info->total_size)
			{
				current_chunk = (void*)heap_info + i;
	...
	else if (heap_info->heap_type == FT_MAL_SMALL_HEAP_TYPE)
	{
		...
		current_chunk->size = heap_info->total_size - (i + FT_MAL_CHUNK_SIZE);
...
t_s_ft_mal_heap_info	*ft_mal_new_heap(t_s_ft_mal_state *arena, t_e_ft_mal_heap_type heap_type, size_t large_chunk_size)
{
	...
	ft_bzero((void*)heap_info, FT_MAL_HEAP_INFO_SIZE);
	...
	if (!arena)
	{
		...
		ft_bzero(arena, FT_MAL_STATE_SIZE);
	}
